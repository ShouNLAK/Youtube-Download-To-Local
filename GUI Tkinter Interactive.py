...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        def update_quality_options(*args):
            ...
            if fmt == 'mp4':
                ...
                # prepare storage for parallel format ids
                ...
                if not url or not (url.startswith('http') or 'youtube.com' in url or 'youtu.be' in url):
                    # nothing to fetch yet - show Auto
                    try:
                        self.quality_combo['values'] = ['Auto']
                        self.quality_combo.set('Auto')
                        self.quality_format_ids = []
                        self.quality_combo.pack(side='left', padx=(8,4))
                    except Exception:
                        pass
            ...
            if inp.lower().startswith('http') or 'youtube.com' in inp or 'youtu.be' in inp:
                ...
                try:
                    if it.fmt == 'mp4' and getattr(self, 'quality_combo', None):
                        ...
                        if sel and sel != 'Auto' and getattr(self, 'quality_format_ids', None):
                            try:
    ...
    def _on_tree_select(self, event=None):
        ...
        # If format is mp4, try to populate toolbar quality options from selected item
        try:
            if self.format_var.get() == 'mp4' and getattr(item, 'url', None):
    ...
    def _open_player_window(self, item: DownloadItem):
        ...
        class PlayerWindow(tk.Toplevel):
            def __init__(self, master, media_url, title=None):
                ...
                # set window handle after small delay
                self.after(200, self._set_hwnd)
    ...
    def _open_player_window_with_url(self, item: DownloadItem, url: str):
        ...
        class PlayerWindow(tk.Toplevel):
            def __init__(self, master, media_url, title=None):
                ...
                try:
                    ...
                    for f in fmts:
                        ...
                        collected.append((height_int, 1 if has_audio else 0, label, out_fmt, f.get('url')))
                    # sort by height desc then audio presence
                    collected.sort(key=lambda x: (x[0], x[1]), reverse=True)
                    self.quality_options = [c[2] for c in collected]
                    self.quality_format_ids_local = [c[3] for c in collected]
                    self.quality_urls_local = [c[4] for c in collected]
                ...
                try:
                    # prefer using the format id (will be resolved by set_media)
                    if getattr(self, 'quality_format_ids_local', None):
                        self.set_media(self.quality_format_ids_local[0])
                    elif getattr(self, 'quality_urls_local', None):
                            ...
                            with youtube_dl.YoutubeDL(ydl_opts) as ydl:
                                ...
                                if info:
                                    ...
                                    if direct:
                                        ...
                                        fmts = info.get('formats') or []
                                        if fmts:
                                            # pick first available candidate with url
                                            for f in fmts:
                                                if f.get('url'):
                                                    stream = f.get('url')
                                                    break
            ...
            def on_quality_change(self, event=None):
                ...
                try:
                    fmt_expr = None
                    if getattr(self, 'quality_format_ids_local', None) and idx < len(self.quality_format_ids_local):
                        fmt_expr = self.quality_format_ids_local[idx]
                    elif getattr(self, 'quality_urls_local', None) and idx < len(self.quality_urls_local):
                        fmt_expr = self.quality_urls_local[idx]
                    else:
                        return
                except Exception:
                    return
                    ...
                    def seek_and_play():
                        try:
                            if cur_pos and cur_pos > 0:
                                self.player.play()
                                # allow some time for media to be ready
                                self.after(300, lambda: self.player.set_time(cur_pos))
                                self.play_state.set('Pause')
                                self._is_playing = True
                            else:
                                self.player.play()
    ...
    def _resolve_stream_url(self, item: DownloadItem):
        ...
        best = candidates[0][1]
        try:
            self.event_q.put(('log', f"_resolve_stream_url: chosen protocol={best.get('protocol')} ext={best.get('ext')} url={'(trimmed)' if best.get('url') and len(best.get('url'))>200 else best.get('url')}"))
        except Exception:
            pass
        return best.get('url')
    ...
    def _search_and_add(self, query: str, max_results: int = 20):
        ...
        def show_results():
            ...
            # Set window size to fit 2 rows of 5 tiles (10 per page)
            dlg.geometry('1200x680')


            # Log search fetch complete
            self.event_q.put(('log', f"Search fetched {len(results)} entries for query: '{query}'"))

            header = ttk.Frame(dlg)
            header.pack(fill='x', padx=8, pady=(8, 4))
            title_lbl = ttk.Label(header, text=f"Search results for: {query}", font=('Segoe UI', 11, 'bold'))
            ...
            ref_frame = ttk.Frame(dlg)
            ref_frame.pack(fill='x', padx=8)
            ttk.Label(ref_frame, text='Refine:').pack(side='left')
            refine_entry = ttk.Entry(ref_frame, textvariable=refine_var)
            refine_entry.pack(side='left', fill='x', expand=True, padx=(6,6))
            ttk.Button(ref_frame, text='Clear', command=lambda: refine_var.set('')).pack(side='left')

            # Use a frame with grid layout for better fit and responsiveness
            grid_frame = ttk.Frame(dlg)
            grid_frame.pack(fill='both', expand=True, padx=8, pady=8)

            tiles = []
            seen_ids = set()

            # progress bar while thumbnails load
            ...
            def make_tile(parent, entry, idx):
                frame = tk.Frame(parent, relief='raised', bd=1, bg='white', width=220, height=170)
                frame.grid_propagate(False)
                # Show title immediately
                title = entry.get('title') or entry.get('id')
                ttl_lbl = tk.Label(frame, text=title, wraplength=200, justify='center', bg='white', font=('Segoe UI', 9, 'bold'))
                ttl_lbl.pack(pady=(8,2))
                # Show uploader and duration
                uploader = entry.get('uploader') or ''
                dur = format_duration(entry.get('duration'))
                meta_lbl = tk.Label(frame, text=f"{uploader} {('• ' + dur) if dur else ''}", wraplength=200, justify='center', bg='white', foreground='#444', font=('Segoe UI', 8))
                meta_lbl.pack(pady=(0,6))
                # Thumbnail placeholder with animation
                thumb_lbl = tk.Label(frame, text='', bg='white')
                thumb_lbl.pack(pady=(0,4))
                # Animated loading indicator
                loading_frames = ['⏳','⌛','⏳','⌛']
                loading_idx = [0]
                def animate_loading():
                    thumb_lbl.config(text=loading_frames[loading_idx[0] % len(loading_frames)])
                    loading_idx[0] += 1
                    thumb_lbl.after(400, animate_loading)
                animate_loading()

                selected = {'v': False}

                def set_selected(val: bool):
                    selected['v'] = val
                    color = '#cce5ff' if val else 'white'
                    try:
                        frame.config(bg=color)
                        thumb_lbl.config(bg=color)
                        ttl_lbl.config(bg=color)
                        meta_lbl.config(bg=color)
                    except Exception:
                        pass
                    try:
                        any_selected = any(s['v'] for (_, s, _, _, _) in tiles)
                        if any_selected:
                ...
                def toggle(event=None):
                    set_selected(not selected['v'])

                def add_single(event=None):
                ...
                ttl_lbl.bind('<Button-1>', toggle)
                meta_lbl.bind('<Button-1>', toggle)
                frame.bind('<Double-Button-1>', add_single)
                ...
                def fetch_thumb_task():
                    url = entry.get('thumbnail')
                    if not url:
                        self.event_q.put(('log', f"No thumbnail for {entry.get('title') or entry.get('id')}"))
                        return
                    self.event_q.put(('log', f"Starting thumbnail fetch for: {entry.get('title') or entry.get('id')}"))
                    try:
                        import urllib.request, io
                        ...
                        if Image is not None and ImageTk is not None:
                            ...
                            def put_img():
                                try:
                                    ...
                                    thumb_lbl.image = img
                                    self.event_q.put(('log', f"Thumbnail loaded: {entry.get('title') or entry.get('id')}"))
                                except Exception:
                                    pass
                            self.event_q.put(('show_dialog', put_img))
                    except Exception as e:
                        try:
                            self.event_q.put(('log', f"Thumbnail fetch failed for {entry.get('title') or entry.get('id')}: {e}"))
                        except Exception:
                            pass
                # return constructed tile tuple
                return (entry, selected, thumb_lbl, title, frame)

            # create tiles for all results but do not grid them yet
            for i, e in enumerate(results):
                t = make_tile(grid_frame, e, i)
                tiles.append(t)

            def layout_tiles(visible_tiles):
                # Place tiles in a 2x5 grid (10 per page)
                for i, (entry, selected, thumb_lbl, title, frame) in enumerate(tiles):
                    try:
                        if (entry, selected, thumb_lbl, title, frame) in visible_tiles:
                            idx = visible_tiles.index((entry, selected, thumb_lbl, title, frame))
                            r = idx // 5
                            c = idx % 5
            ...
            def render_page(page_num: int):
                ...
                current_page['n'] = page_num
                start = (page_num - 1) * PAGE_SIZE
                end = start + PAGE_SIZE
                visible = tiles[start:end]
                layout_tiles(visible)
                page_lbl.config(text=f"Page {page_num} / {total_pages}")
                self.event_q.put(('log', f"Rendered page {page_num} (items {start+1}-{min(end, total_results)})"))

            # reflow when grid_frame resized
            grid_frame.bind('<Configure>', lambda e: dlg.after(100, lambda: render_page(current_page['n'])))
            dlg.after(120, lambda: render_page(current_page['n']))

            # selection helpers
            def select_all():
                for entry, selected, thumb_lbl, title, frame in tiles:
                    try:
                        selected['v'] = True
                ...
                try:
                    ...
                    pass
                self.event_q.put(('log', 'Select All clicked'))
            ...
            def clear_selection():
                ...
                try:
                    ...
                    pass
                self.event_q.put(('log', 'Clear Selection clicked'))

            def on_refine(*a):
                q = refine_var.get().strip().lower()
                # show/hide entries based on refine and then recompute pages
                filtered = []
                for entry, selected, thumb_lbl, title, frame in tiles:
                    text = (entry.get('title') or '') + ' ' + (entry.get('uploader') or '')
                    visible_flag = (q in text.lower()) if q else True
                    if visible_flag:
                        filtered.append((entry, selected, thumb_lbl, title, frame))
                # temporarily compute pages for filtered set
                # We'll render the first page of the filtered results by hiding non-matching tiles
                layout_tiles(filtered[:PAGE_SIZE])
                page_lbl.config(text=f"Filter results: {len(filtered)} shown")
                self.event_q.put(('log', f"Refine applied: '{q}' – {len(filtered)} matches"))

            refine_var.trace_add('write', lambda *a: on_refine())
            ...
            def on_add_selected():
                ...
                for entry, selected, thumb_lbl, title, frame in tiles:
                    if selected['v']:
                        url = entry.get('webpage_url') or f"https://www.youtube.com/watch?v={entry.get('id')}"
                        it = DownloadItem(url, fmt=self.format_var.get(), bitrate=self.bitrate_var.get())
                        # attach selected toolbar quality if mp4
                        try:
                            if it.fmt == 'mp4' and getattr(self, 'quality_combo', None):
                                sel = self.quality_combo.get()
                                if sel and sel != 'Auto' and getattr(self, 'quality_format_ids', None):
                        ...
                        self.tree.insert('', 'end', iid=iid, values=(it.title or it.url, it.status, self._format_progress_bar(it.progress)))
                        added += 1
                        self.event_q.put(('log', f'Added from search: {it.title or it.url}'))
                dlg.destroy()
                try:
                    self._draw_progress_overlays()
                except Exception:
                    pass
                self.event_q.put(('log', f'Add Selected finished - {added} items added'))

            # control frame with pagination
            ctrl = ttk.Frame(dlg)
            ctrl.pack(fill='x', padx=8, pady=(0,8))
            add_btn = ttk.Button(ctrl, text='Add Selected', command=on_add_selected)
            add_btn.pack(side='left')
            add_btn.state(['disabled'])
            load_more_btn = ttk.Button(ctrl, text='Load more', command=lambda: dlg.after(10, load_more))
            load_more_btn.pack(side='left', padx=6)
            # pagination controls
            prev_btn = ttk.Button(ctrl, text='◀ Prev', command=lambda: (render_page(current_page['n'] - 1)))
            prev_btn.pack(side='left', padx=6)
            page_lbl = ttk.Label(ctrl, text=f"Page {current_page['n']} / {total_pages}")
            page_lbl.pack(side='left')
            next_btn = ttk.Button(ctrl, text='Next ▶', command=lambda: (render_page(current_page['n'] + 1)))
            next_btn.pack(side='left', padx=6)
            ttk.Button(ctrl, text='Select All', command=select_all).pack(side='left', padx=6)
            ttk.Button(ctrl, text='Clear', command=clear_selection).pack(side='left')
            ttk.Button(ctrl, text='Cancel', command=dlg.destroy).pack(side='right')

            def load_more():
                nonlocal results, total_results, total_pages
                # request more results (increase requested count) and append new non-duplicate tiles
                try:
                    ...
                    # replace results with expanded list
                    results = more
                    added = 0
                    for e in results:
                        vid = e.get('id')
                        if not vid or vid in seen_ids:
                            continue
                        t = make_tile(grid_frame, e, len(tiles))
                        tiles.append(t)
                        added += 1
                        if added >= PAGE_SIZE:
                            break
                    ...
                    total_results = len(results)
                    total_pages = max(1, (total_results + PAGE_SIZE - 1) // PAGE_SIZE)
                    render_page(current_page['n'])
                    count_lbl.config(text=f"{len(results)} results")
                    self.event_q.put(('log', f'Load more completed - {added} new tiles added'))
                    # Do not disable Load more; always allow user to fetch more
                except Exception as e:
                    self.event_q.put(('log', f'Load more failed: {e}'))

            # stop progress bar after small delay (thumbnails fetch in background)
            def stop_progress():
                try:
                    progress.stop()
                    progress.destroy()
                    self.event_q.put(('log', 'Thumbnail loading background tasks started'))
                except Exception:
                    pass
            dlg.after(1200, stop_progress)
